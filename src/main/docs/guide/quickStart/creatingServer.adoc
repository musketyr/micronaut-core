...
By default the Micronaut HTTP server is configured to run on port 8080, but you can alter that by adding the following configuration to `src/main/resources/application.yml`:
...
TIP: This is also possible to be configured from an environment variable: `MICRONAUT_SERVER_PORT=5000`
